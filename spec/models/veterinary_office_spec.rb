...
RSpec.describe VeterinaryOffice, type: :model do
  before do
    @vet_office_1 = described_class.create(name: 'Best Vets', boarding_services: true, max_patient_capacity: 20)
    @vet_office_2 = described_class.create(name: 'Vets R Us', boarding_services: true, max_patient_capacity: 20)
  ...
  describe 'class methods' do
    describe '#search' do
      it 'returns partial matches' do
        expect(described_class.search('Vets')).to eq([
    ...
    describe '#order_by_recently_created' do
      it 'orders the offices by those most recently created first' do
        expect(described_class.order_by_recently_created).to eq([
    ...
    describe '#order_by_number_of_vets' do
      it 'orders the offices by those with most veterinarians to least' do
        expect(described_class.order_by_number_of_vets).to eq([
                                                                @vet_office_1, @vet_office_2
                                                              ])
      end
    end
